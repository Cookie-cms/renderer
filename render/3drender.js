...
class Render3d {
    ...
    constructor(imageBufferFile, isAlex = false, imageWidth = Render3d.side, capeBufferFile = null) {
    ...
    if (capeBufferFile) {
        this.capeBuffer = capeBufferFile;
        const capeFile = new Image();
        capeFile.src = capeBufferFile;
        this.capeImage = capeFile;

        // Validate cape dimensions
        if (!capeFile.width || typeof capeFile.width !== "number") 
            throw Error(`Invalid cape file dimensions! Width was ${capeFile.width}.`);
        if (capeFile.width !== 64)
            throw Error(`Invalid cape file width! Expected 64, got ${capeFile.width}`);
        if (capeFile.height !== 32)
            throw Error(`Invalid cape file height! Expected 32, got ${capeFile.height}`); 
        }
    }
    ...
    _renderCape(ctx, side, baseOffsetL, baseOffsetT) {
        if (!this.capeImage) return;
        ...
        ctx.transform(
            1, -0.5, 0, 1,
            baseOffsetL + side * 0.5,
            baseOffsetT + side * 1.5
        );
        ctx.drawImage(
            this.capeImage,
            1, 1,                    // Source x, y
            10, 16,                  // Source width, height
            0, 0,                    // Destination x, y
            w * 0.8, h * 1.2        // Destination width, height
        );
    
        // Draw cape sides if visible
        ctx.transform(
            1, 0.5, 0, 1,
            -w * 0.4,
            h * 0.2
        ...
        ctx.drawImage(
            this.capeImage,
            0, 1,                    // Source x, y
            1, 16,                   // Source width, height
            0, 0,                    // Destination x, y
            w * 0.1, h * 1.2        // Destination width, height
        );
    ...
    this._canvas = createCanvas(imageWidth, imageWidth);
        let ctx = tmpCanvas.getContext("2d");

        if (this.capeImage) {
            this._renderCape(ctx, side, baseOffsetL, baseOffsetT);
        }
